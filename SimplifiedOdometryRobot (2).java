...
public class SimplifiedOdometryRobot {
    ...
    public boolean readSensors() {
        ...
        int yCounts = pinpoint.getEncoderY();
        int xCounts = pinpoint.getEncoderX();
...
/*
    public void drive(double distanceInches, double maxPower, double holdTime) {
        resetOdometry();  // set current wheels as zero

        driveController.reset(distanceInches, maxPower);   // target forward/back distance
        strafeController.reset(0.0, maxPower);             // maintain zero lateral drift
        yawController.reset(heading);                      // hold current heading
        holdTimer.reset();
        ...
        while (myOpMode.opModeIsActive() && readSensors()) {
            ...
            double yawPower    = yawController.getOutput(heading);  // keep robot facing same way
    ...
    public void strafe(double distanceInches, double maxPower, double holdTime) {
        resetOdometry();  // set current wheels as zero

        driveController.reset(0.0, maxPower);             // maintain zero forward/back drift
        strafeController.reset(distanceInches, maxPower); // target lateral distance
        yawController.reset(heading);                     // hold current heading
        holdTimer.reset();
